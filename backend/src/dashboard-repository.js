...
function getNationalSummary(db) {
  return db
    .prepare(`
      ...
      FROM packages
      WHERE owner_name LIKE '%Bandung Barat%'  -- Filter Tambahan
    `)
    .get();
}
...
function getRegionRows(db) {
  return db
    .prepare(`
      ...
      WHERE regions.display_name LIKE '%Bandung Barat%' -- Filter Tambahan
      ORDER BY
  ...
  const rows = db
    .prepare(`
      ...
      ORDER BY
        ...
        (packages.budget < 200000000) DESC, -- Tambahkan baris ini
...
function queryOwnerPackagesPage(db, ownerType, ownerName, normalizedQuery) {
  ...
  const rows = db
    .prepare(`
      ...
      ORDER BY
        packages.is_priority DESC,
        (packages.budget < 200000000) DESC,
        packages.potential_waste DESC,
        packages.risk_score DESC,
        COALESCE(packages.budget, 0) DESC,
        packages.inserted_order ASC
